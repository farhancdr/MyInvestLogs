...
![Tests](https://img.shields.io/badge/tests-59%20unit%20%2B%2042%20e2e-success)
...
npm run test:e2e   # 42 Playwright tests — the real app in a browser
npm run typecheck
```
...
The 59 unit tests encode worked examples of each accounting rule, so changing a rule breaks a test by name rather than silently shifting a total. The 42 Playwright tests drive the real app against a seeded database — including that voiding preserves the original row, that future-dated transactions are rejected, and that profit-share investments render N/A rather than a fabricated expectation.
